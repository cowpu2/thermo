...
##  Works as of 20180308 - relays are burnt up - try other relays
...
def GetTemps():
    for sensor in W1ThermSensor.get_available_sensors():
        ...
        except W1ThermSensorError(NoSensorFoundError) as e:
            Sand_temp = e
            continue
        try:
...
def DataLogger():  ##  Create and write to log file
    ...
    logdatastring = str(round(Sand_temp, 2)) + ", " + str(round(Soil_temp, 2)) + ", " + str(
        round(Air_temp, 2)) + ", " + str(round(Outside_temp, 2) + ", " + float(rpi_temp) + ", " + TempStatus)
